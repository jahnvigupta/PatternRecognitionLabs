# -*- coding: utf-8 -*-
"""Q1_Assignment1_PR.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1v-xOX0d5t5Hkq-wGOFF5nt3Mbru6hzFD
"""
...
#probability function for linear separation
def prob_lin(n, mu, covar, vec, total_samp):
  temp_mat = (-1/2)*(np.matmul(np.transpose(vec - mu), np.linalg.inv(covar)))
  temp1 = np.matmul(temp_mat, vec - mu)
  temp2 = (-1/2)*np.log(np.linalg.det(covar))
  temp3 = np.log(n/total_samp)
  temp4 = (1/2)*(np.matmul(np.transpose(vec), np.linalg.inv(covar)))
  temp5 = np.matmul(temp4, vec)
  prob = temp1+temp2+temp3+temp5
  return prob
...
#Getting correct predictions in class1
for i in range(len(class1_test)):
  prob1 = prob_lin(len(class1_train),mean1,cov1,class1_test.iloc[i],len(class1_train)+len(class2_train))
  prob2 = prob_lin(len(class2_train),mean2,cov2,class1_test.iloc[i],len(class1_train)+len(class2_train))
  if(prob1>prob2):
    corr_pred_lin = corr_pred_lin+1

#Getting correct predictions in class2
for i in range(len(class2_test)):
  prob1 = prob_lin(len(class1_train),mean1,cov1,class2_test.iloc[i],len(class1_train)+len(class2_train))
  prob2 = prob_lin(len(class2_train),mean2,cov2,class2_test.iloc[i],len(class1_train)+len(class2_train))
  if(prob1<prob2):
    corr_pred_lin = corr_pred_lin+1

#Printing accuracy
print("Accuracy for linear separation is : ",corr_pred_lin/(len(class1_test)+len(class2_test)))